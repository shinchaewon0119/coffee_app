...
def get_conn():
    """
    기존 코드 호환용 함수.
    SQLite에서는 conn을 반환했지만, Supabase에서는 별도 conn이 필요 없음.
    다른 코드가 get_conn()을 호출하고 있어도 깨지지 않게 None 반환.
    """
    return None
...
def init_db(conn):
    """
    기존 코드 호환용 함수.
    Supabase에서는 테이블을 SQL Editor에서 미리 만들기 때문에 여기서는 아무 작업도 하지 않음.
    """
    return None
...
def load_profile(conn, user_id):
    ...
    try:
        result = (
            ...
            .select("acidity,sweet,bitter,body,flavor,long_tag_profile_json")
        ...
        long_profile = {
            "acidity": float(row.get("acidity") or 3.0),
            "sweet": float(row.get("sweet") or 3.0),
            "bitter": float(row.get("bitter") or 3.0),
            "body": float(row.get("body") or 3.0),
            "flavor": float(row.get("flavor") or 3.0),
        }

        try:
            long_tag = json.loads(row.get("long_tag_profile_json") or "{}")
        except Exception:
            long_tag = {}
        ...
        st.warning(f"프로필 불러오기 중 오류가 발생했습니다: {e}")
        return dict(DEFAULT_LONG_PROFILE), {}


def profile_exists(conn, user_id):
    """
    user_profile에 행이 있으면 True.
    기존 콜드스타트 판단 로직 유지.
    """
    try:
        result = (
        ...
        st.warning(f"프로필 확인 중 오류가 발생했습니다: {e}")
        return False
...
def save_profile(conn, user_id, long_profile, long_tag):
    """
    Supabase에 장기 프로필 저장.
    SQLite의 ON CONFLICT 업데이트와 같은 역할을 upsert로 처리.
    """
    data = {
        "user_id": str(user_id),
        "acidity": float(long_profile.get("acidity", 3.0)),
        "sweet": float(long_profile.get("sweet", 3.0)),
        "bitter": float(long_profile.get("bitter", 3.0)),
        "body": float(long_profile.get("body", 3.0)),
        "flavor": float(long_profile.get("flavor", 3.0)),
        "long_tag_profile_json": json.dumps(long_tag, ensure_ascii=False),
        "updated_at": datetime.now().isoformat(),
    }

    try:
        supabase.table("user_profile").upsert(data, on_conflict="user_id").execute()

    except Exception as e:
        st.warning(f"프로필 저장 중 오류가 발생했습니다: {e}")


def log_behavior(conn, user_id, product_id, bean_name, action, session_id):
    """
    Supabase에 사용자 행동 로그 저장.
    기존 함수 호출 구조 유지:
    log_behavior(conn, user_id, product_id, bean_name, action, session_id)
    """
    data = {
        "user_id": str(user_id),
        "product_id": str(product_id),
        "bean_name": str(bean_name),
        "action": str(action),
    ...
    try:
        supabase.table("behavior_logs").insert(data).execute()

    except Exception as e:
        st.warning(f"행동 로그 저장 중 오류가 발생했습니다: {e}")


def get_logs_by_product(conn, user_id):
    """
    Supabase에서 특정 user_id의 행동 로그를 불러와
    기존 코드에서 쓰던 딕셔너리 형태로 반환.

    반환 예:
    {
        "product_1": [
            {"action": "like", "session_id": 1},
            {"action": "cart", "session_id": 1}
        ]
    }
    """
    try:
        result = (
            supabase.table("behavior_logs")
            .select("product_id, action, session_id")
            .eq("user_id", str(user_id))
            .execute()
        )
        ...
        st.warning(f"행동 로그 불러오기 중 오류가 발생했습니다: {e}")
        return {}


def current_session_id(conn, user_id):
    """
    기존 SQLite에서는 MAX(session_id)+1로 현재 세션 번호를 만들었음.
    Supabase에서도 같은 방식 유지.
    """
    try:
        result = (
            supabase.table("behavior_logs")
        ...
        if result.data and result.data[0].get("session_id") is not None:
            return int(result.data[0]["session_id"]) + 1

        return 1

    except Exception as e:
        st.warning(f"세션 ID 생성 중 오류가 발생했습니다: {e}")
        return 1


def update_long_profile(conn, user_id, conds, long_profile, long_tag):
    """
    현재 세션에서 드러난 취향을 장기 프로필에 EMA로 반영.
    기존 로직 유지.
    """
...
def main():
    ...
    if debug_mode:
        with st.expander("🧪 Debug: 정책/필터 검증 패널", expanded=True):
            ...
            result = (
                supabase.table("behavior_logs")
                .select("bean_name, action, session_id, created_at")
                .eq("user_id", str(user_id))
                .order("id", desc=True)
                .limit(10)
                .execute()
